...
func stxnCoreChecks(gi int, groupCtx *GroupContext, batchVerifier crypto.BatchVerifier) *TxGroupError {
	...
	if s.Txn.Type == protocol.HeartbeatTx && s.Txn.Group.IsZero() {
		var keyDilution uint64      // = s.Txn.HbKeyDilution
		var voteID crypto.PublicKey // = s.Txn.HbVoteID
		id := basics.OneTimeIDForRound(s.Txn.LastValid, keyDilution)
		...
		batchVerifier.EnqueueSignature(voteID, batchID, crypto.Signature(s.Txn.HbProof.PK2Sig))
