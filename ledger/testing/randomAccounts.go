...
// RandomAssetParams creates a random non-empty asset params.
// freeze flag forces DefaultFrozen to be set so all AssetParams fields have non-empty values.
func RandomAssetParams(freeze bool) basics.AssetParams {
...
// RandomAssetHolding creates a random non-empty asset holding
// freeze flag forces Frozen to be set so all AssetParams fields have non-empty values.
func RandomAssetHolding(freeze bool) basics.AssetHolding {
...
// RandomAppParams creates a random non-empty app params
		...
		for addr, old := range base {
			...
			if simple {
				...
				for aidx, params := range data.AppParams {
					updates.UpsertAppParams(addr, aidx, &params)
				}
				for aidx, states := range data.AppLocalStates {
					updates.UpsertAppLocalState(addr, aidx, &states)
				}
				for aidx, params := range data.AssetParams {
					updates.UpsertAssetParams(addr, aidx, &params)
				}
				for aidx, holding := range data.Assets {
					updates.UpsertAssetHolding(addr, aidx, &holding)
				}
			}
