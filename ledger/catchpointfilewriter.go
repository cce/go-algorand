...
type catchpointFileWriter struct {
	...
	kvRows                 kvIter
	kvDone                 bool
	onlineAccountRows      onlineAccountsIter
	onlineAccountsDone     bool
	onlineRoundParamsRows  onlineRoundParamsIter
...
func (cw *catchpointFileWriter) readDatabaseStep(ctx context.Context) error {
	...
	if !cw.onlineAccountsDone {
		...
		for cw.onlineAccountRows.Next() {
			oa, err := cw.onlineAccountRows.OnlineAccount()
			if err != nil {
				return err
			}
	...
	if !cw.onlineRoundParamsDone {
		...
		for cw.onlineRoundParamsRows.Next() {
			or, err := cw.onlineRoundParamsRows.OnlineRoundParams()
			if err != nil {
				return err
			}
