...
// PreallocatedKeys returns the keys of the map m, and uses a
// preallocated slice to collect them, like the Keys method in
// golang.org/x/exp/maps package (but not builtin "maps" package).
// The keys will be in an indeterminate order.
func PreallocatedKeys[M ~map[K]V, K comparable, V any](m M) []K {
	r := make([]K, 0, len(m))
	for k := range m {
		r = append(r, k)
	}
